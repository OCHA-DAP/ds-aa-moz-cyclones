...
gdf_points_sel_["REU_CAT"] = [
    helpers.categorize_storm_knots(speed)
    for speed in gdf_points_sel_["REU_USA_WIND"]
]
```
...
moz_emdat = emdat_file[
    (emdat_file["Country"] == "Mozambique")
    & (emdat_file["Disaster Type"] == "Storm")
]
```

```python
moz_cerf = cerf_file[
    (cerf_file["Country"] == "Mozambique")
    & (cerf_file["Emergency"] == "Storm")
]
```
...
merged_df_clean = merged_df_clean[
    merged_df_clean["SID"].str[:4].astype(int) >= 1998
]
...
plt.xlabel("Values", fontweight="bold")
plt.title(
    "Total Affected and Money Allocated by Cyclone (Category while on land)"
)
...
labels_with_initials = [
    ...
    for name, category in zip(
        merged_df_clean["NAME"], merged_df_clean["REU_CAT"]
    )
]
